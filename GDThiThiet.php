...
// mảng lưu các index đã được random 
if (!isset($_SESSION['chosenNumbers'])) {
    $_SESSION['chosenNumbers'] = [];
}

// random ngẫu nhiên 25 câu hỏi
for($i = 0; $i < 25; $i++){
	if(!isset($_SESSION['randomQuestion'][$i])){
		// kiểm tra 1 số đã được random hay chưa?
		do {
			$randomQuestion = rand(1, 200);
		} while (in_array($randomQuestion, $_SESSION['chosenNumbers']));

		// Thêm số đã random vào mảng
		$_SESSION['chosenNumbers'][] = $randomQuestion;
	
		// Thực hiện truy vấn SQL	
		$dataQuery = "SELECT CauHoi, CauA, CauB, CauC, CauD, DapAn, HinhAnh, CauHoiLiet FROM cauhoi where SttCau=".$randomQuestion." ";
		...
		while ($row = mysqli_fetch_assoc($resultQuery)) {
			...
			// Thêm  dư liệu của câu hỏi vào ss
			$_SESSION['randomQuestion'][$i] = $question;
			$_SESSION['answerCorrects'][$i] = $row['DapAn'];
			$_SESSION['checkDiemLiet'][$i] = $row['CauHoiLiet'];
		}
	}
}
	...
	<div>
		...
		<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" id="radioForm">
			...
			<?php for ($i = $startIndex; $i <= $endIndex; $i++){
					if(isset($_SESSION['randomQuestion'][$i])){ // nếu tồn tại câu hỏi này
			...
			<div class="right-body">
                ...
                <div class="right-time" id="countdown">
                    <script>
                        ...
                        //đồng hồ đếm ngược thời gian trên gd người dùng 
                        function run_clock(id, endtime) {
                            ...
                            update_clock();
							//thiết lập một khoảng thời gian để cập nhật đồng hồ mỗi giây
					...
					<div class="panel-body">
						<div align="right" colspan="2">
							...
							for ($page = 1; $page <= $totalPages; $page++) {
								$is_checked = (isset($_SESSION['selectedAnswer'][$page-1])) ? "checked" : ""; // Kiểm tra câu hỏi này đã trả lời chưa
		...
		<div>
			<form action="GDketqua.php" method="POST">
				...
				?>
				<button type="submit" class="btn-submit" id="submit_button" name="ketquathithiet" value="<?php echo $ketQuaThi[0] . ',' . $ketQuaThi[1]; ?>">Nộp bài</button>
			</form>
		</div>
	</div>
	<!-- End container -->		
...
function sumQuestionCorrect(){
    $countCorrect = 0; // đếm số câu hỏi trả lời đúng
    ...
    for($i = 0; $i <= 24; $i++){
        ...
        if($_SESSION['checkDiemLiet'][$i] == 01){
			// nếu thí sinh đã chọn câu trả lời cho câu hỏi thứ $i
			if(isset($_SESSION['selectedAnswer'][$i]) && isset($_SESSION['answerCorrects'][$i])){
				// nếu câu trả lời của câu hỏi liệt khác đáp án thì tăng số câu hỏi liệt trả lời sai lên 1 đơn vị    
				if($_SESSION['selectedAnswer'][$i] != $_SESSION['answerCorrects'][$i]){
					$countIncorrectDiemLiet++;
				}
				// ngược lại tăng số câu hỏi trả lời đúng
				else{
					$countCorrect++;
				}
			}
		...
		// không là câu hỏi liệt 
			...
			// nếu câu trả lời trùng với đáp án thì tăng số câu trả lời đúng lên 1 đơn vị
	...
	// trả về dưới dạng mảng tổng số câu hỏi trả lời đúng và tổng số câu hỏi liệt trả lời sai
